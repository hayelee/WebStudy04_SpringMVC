...
public class BootstrapPaginationRender implements PaginationRenderer {
	...
	public String renderPagination(PagingVO<?> pagingVO) {
		...
		String pagePtrn = "<li class='page-item'><a class='page-link paging' href='#' data-page='%1$d'>%1$d</a></li>"; // 1번페이지
		...
		boolean disabled = startPage <= 1; //if문 대신
	    html.append(
    		String.format(previousPtrn
    					, disabled?"disabled":""
    					, disabled?"":"href='#'"
    					, disabled?1:startPage-1 //이전구간의 마지막페이지로
